...
# 3. Valoración del interés de las variables para el problema y selección de un subconjunto (en su caso)
  ...
  Se realizará el estudio también utilizando únicamente las variables metereológicas y los índices del sistema FWI calculados a partir de las variables disponibles.  

  Se propone finalmente usar el índice FWI, calculado a partir de los índices y variables metereológicas de la BD. Este índice no fue considerado  incluirlo quienes realizaron la base de datos debido a que es composición. Se verá si se tiene una diferencia a tener en cuenta con respecto a los valores que agrupa.
...
```
  ...
  Antes de continuar, instalamos los paquetes que se usarán en el proyecto si fuera necesario hacerlo:
  
```{r, message=FALSE, warning=FALSE}
list.of.packages <- c("glmnet","randomForest","dismo",
                      "nnet","RSNNS","e1071")
