...
SGD <- function(datos, valores, max_iter, vini= c(1,1,1), 
  ...
  while ( iter < max_iter && !stopped){
    ...
    print(norm_v(sol - prev_sol))
    if(norm_v(sol - prev_sol) < 0.01)
      stopped = TRUE
    if(draw_iterations){
      draw_function(c(-1,1), c(-1,1), function(x,y) y +sol[1]/sol[2]*x
                    +sol[3]/sol[2], col = 4)
...
g_4a <- function(x,y) sol_4a$hiperplane[1]*x + sol_4a$hiperplane[2]*y + sol_4a$hiperplane[3]
...
datos_test <- datos_test[-(which(datos_test[,1] != 1 & datos_test[,1]!=5)),]
...
$\textit{Plantear un problema de clasificación binaria que considere el conjunto de entrenamiento como datos de entrada para aprender la función $g$. Usando el modelo de Regresión Lineal para clasificación seguido por PLA-Pocket como mejora. Responder a las siguientes cuestiones.}$
...
coefs_rl_5 <- regress_lin(cbind(means_train,sim_vertical_train,rep(1,length(means_train))), label_5)
coefs_5 <- ajusta_PLA_MOD(cbind(means_train,sim_vertical_train), label_5, 100, coefs_rl_5)
```
...
draw_function(c(-1,0.2),c(-260,10),function(x,y) y + coefs_rl_5[1]/coefs_rl_5[2]*x +coefs_rl_5[3]/coefs_rl_5[2])
draw_function(c(-1,0.2),c(-260,10),function(x,y) y + coefs_5[1]*x +coefs_5[3])
...
draw_function(c(-1,0.2),c(-260,10),function(x,y) y + coefs_5_rl[1]/coefs_5_rl[2]*x +coefs_5[3]/coefs_5_rl[2])
draw_function(c(-1,0.2),c(-260,10),function(x,y) y + coefs_5[1]*x +coefs_5[3])
points(data_list_train$mean, data_list_train$v_symmetry, col = data_list_train$colors )
