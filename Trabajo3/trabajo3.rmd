...
```{r}
train.index <- sample(1:nrow(Boston), size = nrow(Boston)*4/5)
```
...
```{r}
bag.boston = randomForest(medv~., data=Boston, subset=train.index,
                          mtry=13, importance = TRUE )
y.pred.bag = predict( bag.boston, newdata=Boston[-train.index,])
plot ( y.pred.bag, Boston[-train.index,"medv"] )
abline (0 ,1)
mean ( (y.pred.bag - Boston[-train.index,"medv"])^2 )
```
...
while( !error.stable ){
  mtry <- tuneRF(Boston[train.index, 
                        -which(names(Boston) %in% "medv")],
                 Boston[train.index, "medv"], ntreeTry = nTrees)
  mtry <- mtry[which.min(mtry[,2]),1]
  bag.boston = randomForest(medv~., data=Boston, subset=train.index,
                            mtry=mtry, importance = T, nTrees=nTrees)
  y.pred.rf = predict( bag.boston, newdata=Boston[-train.index,])
  error.current <- mean((y.pred.rf - Boston[-train.index,"medv"])^2)
  ...
  if( abs(error.prev - error.current) < 0.005){
    error.stable <- TRUE
  }
  else{
...
```{r}
mtry <- tuneRF(Boston[train.index, -which(names(Boston) %in% "medv")],
               Boston[train.index, "medv"], ntreeTry = nTrees)
mtry <- mtry[which.min(mtry[,2]),1]
bag.boston = randomForest(medv~., data=Boston, subset=train.index,
                          mtry=mtry, importance = T, nTrees=nTrees)
y.pred.rf = predict( bag.boston, newdata=Boston[-train.index,])
mean((y.pred.rf - Boston[-train.index,"medv"])^2)
```
...
```{r}
boost.boston = gbm(medv~., data=Boston[train.index,],
...
```{r}
y.pred.boost = predict(boost.boston, newdata=Boston[-train.index,],
                       n.trees = 5000)
mean((y.pred.boost - Boston[-train.index,"medv"])^2)
```


```{r, eval=FALSE}
train(Boston[train.index, -which(names(Boston) %in% "medv")],
      Boston[train.index, "medv"], method = "gbm",
...
```{r}
plot(tree.cv.OJ$size, tree.cv.OJ$dev, type="b",
